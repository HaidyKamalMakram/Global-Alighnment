    ...
    if UserSelect == 1:
        ...
        for i in range(1, Proteinseq2Length + 1, 1):
            for j in range(1, Proteinseq1Length + 1, 1):
                if Seqence1[j - 1] == Seqence2[i - 1]:
                    MatrixScore = b62[(Seqence1[j - 1], Seqence2[i - 1])]
                    print('M= ', b62[((Seqence1[j - 1]), (Seqence2[i - 1]))])
                else:
                    MatrixScore = b62[(Seqence1[j - 1], Seqence2[i - 1])]
                    print('MM= ', b62[(Seqence1[j - 1], Seqence2[i - 1])])
                ProteinLoopMatrix[i][j] = max(ProteinLoopMatrix[i - 1][j] - 1, ProteinLoopMatrix[i][j - 1] - 1, ProteinLoopMatrix[i - 1][j - 1] + MatrixScore)
